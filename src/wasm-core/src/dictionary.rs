...
pub static DICTIONARIES: Lazy<Mutex<HashMap<String, DictionaryStore>>> =
...
pub fn load_dictionary_internal(mode: &str, bin_data: &[u8]) -> Result<(), String> {
    ...
    if bin_data.len() < cursor + 8 {
        return Err("Invalid bin (FST len)".to_string());
    }
    let fst_len = read_u64(&bin_data[cursor..cursor + 8]) as usize;
    cursor += 8;

    if bin_data.len() < cursor + fst_len {
        return Err("Invalid bin (FST body)".to_string());
    }
    ...
    if bin_data.len() < cursor + 8 {
        return Err("Invalid bin (Val len)".to_string());
    }
    let val_len = read_u64(&bin_data[cursor..cursor + 8]) as usize;
    cursor += 8;

    if bin_data.len() < cursor + val_len {
        return Err("Invalid bin (Val body)".to_string());
    }
    let values_bytes = bin_data[cursor..cursor + val_len].to_vec();
    cursor += val_len;

    if bin_data.len() < cursor + 8 {
        return Err("Invalid bin (Suf len)".to_string());
    }
    let suf_len = read_u64(&bin_data[cursor..cursor + 8]) as usize;
    cursor += 8;

    if bin_data.len() < cursor + suf_len {
        return Err("Invalid bin (Suf body)".to_string());
    }
    let suf_bytes = &bin_data[cursor..cursor + suf_len];

    let suffixes: Vec<String> =
        serde_json::from_slice(suf_bytes).map_err(|e| format!("Suffix decode error: {}", e))?;
