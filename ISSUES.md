...
Ovaj dokument služi kao primarni registar svih identifikovanih sistemskih
ograničenja, tehničkog duga i planiranih ispravki.
...
- **Opis:** Trenutna implementacija u `src/shared/ooxml/xmlParser.ts` koristi
  pretraživačev `DOMParser`. Za dokument od 100MB, DOM stablo u RAM-u može
  zauzeti preko 800MB zbog načina na koji V8 mapira čvorove.
- **Simptomi:** Pad Web Workera sa greškom `Out of Memory` ili totalno
  zamrzavanje WebView2 kontrole na računarima sa 8GB RAM-a.
...
- **Opis:** Ako WASM jezgro baci `panic!` (npr. zbog neočekivanog Unicode
...
- **Opis:** Na Windows desktop verziji Word-a, promena sistemske teme
  (Light → Dark) se ne propagira u Taskpane u realnom vremenu. Korisnik vidi
  "stari" UI dok ne klikne na Add-in ili ne uradi reload.
...
- **Opis:** "Bridging" logika u `src/shared/ooxml/bridge` ponekad pogrešno spoji
  karaktere na granici run-ova koji nisu deo iste reči (npr. kraj reči `n` i
...
- **Opis:** Trenutni testovi pokrivaju male OOXML fragmente. Nemamo automatizovan
  test za fajlove od 500+ strana u CI okruženju.
- **Zadatak:** Dodati `tests/stress/` sa generisanim džinovskim XML fajlovima
  (ili generator + pragovi).

### 8) Custom Subs Separator Risk

- **Opis:** Korisnici u "Sopstvene zamene" koriste `->` kao separator. Ako reč
  sadrži taj niz karaktera, parser puca.
