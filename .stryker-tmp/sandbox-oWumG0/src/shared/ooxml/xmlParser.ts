// @ts-nocheck
// src/shared/ooxml/xmlParser.ts
function stryNS_9fa48() {
    var g =
        (typeof globalThis === "object" && globalThis && globalThis.Math === Math && globalThis) ||
        new Function("return this")();
    var ns = g.__stryker__ || (g.__stryker__ = {});
    if (
        ns.activeMutant === undefined &&
        g.process &&
        g.process.env &&
        g.process.env.__STRYKER_ACTIVE_MUTANT__
    ) {
        ns.activeMutant = g.process.env.__STRYKER_ACTIVE_MUTANT__;
    }
    function retrieveNS() {
        return ns;
    }
    stryNS_9fa48 = retrieveNS;
    return retrieveNS();
}
stryNS_9fa48();
function stryCov_9fa48() {
    var ns = stryNS_9fa48();
    var cov =
        ns.mutantCoverage ||
        (ns.mutantCoverage = {
            static: {},
            perTest: {},
        });
    function cover() {
        var c = cov.static;
        if (ns.currentTestId) {
            c = cov.perTest[ns.currentTestId] = cov.perTest[ns.currentTestId] || {};
        }
        var a = arguments;
        for (var i = 0; i < a.length; i++) {
            c[a[i]] = (c[a[i]] || 0) + 1;
        }
    }
    stryCov_9fa48 = cover;
    cover.apply(null, arguments);
}
function stryMutAct_9fa48(id) {
    var ns = stryNS_9fa48();
    function isActive(id) {
        if (ns.activeMutant === id) {
            if (ns.hitCount !== void 0 && ++ns.hitCount > ns.hitLimit) {
                throw new Error("Stryker: Hit count limit reached (" + ns.hitCount + ")");
            }
            return true;
        }
        return false;
    }
    stryMutAct_9fa48 = isActive;
    return isActive(id);
}
import { isSafeXml } from "./xmlSafety";
export function parseSafeOoxml(xml: string): Document | null {
    if (stryMutAct_9fa48("4492")) {
        {
        }
    } else {
        stryCov_9fa48("4492");
        if (
            stryMutAct_9fa48("4495")
                ? false
                : stryMutAct_9fa48("4494")
                  ? true
                  : stryMutAct_9fa48("4493")
                    ? isSafeXml(xml)
                    : (stryCov_9fa48("4493", "4494", "4495"), !isSafeXml(xml))
        ) {
            if (stryMutAct_9fa48("4496")) {
                {
                }
            } else {
                stryCov_9fa48("4496");
                return null;
            }
        }
        try {
            if (stryMutAct_9fa48("4497")) {
                {
                }
            } else {
                stryCov_9fa48("4497");
                const parser = new DOMParser();
                return parser.parseFromString(
                    xml,
                    stryMutAct_9fa48("4498") ? "" : (stryCov_9fa48("4498"), "application/xml")
                );
            }
        } catch {
            if (stryMutAct_9fa48("4499")) {
                {
                }
            } else {
                stryCov_9fa48("4499");
                return null;
            }
        }
    }
}
